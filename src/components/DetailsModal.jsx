// src/components/DetailsModal.jsx - Caminhos corrigidos
import React from 'react';
import { useFavorites } from '../contexts/FavoritesContext'; // Caminho correto
...
const fetchDetails = async ({ queryKey }) => {
    ...
    if (!item || !TMDB_API_KEY) return null;
...
const DetailsModal = ({ item, onClose, onPlay }) => {
    ...
    const { data: details, isLoading, isError } = useQuery({
        ...
        enabled: !!item && !!item.tmdb_id && item.tmdb_id !== '0' && !!TMDB_API_KEY,
    });
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-80 flex justify-center items-center z-50 p-4" onClick={onClose}>
            <div className="relative w-full h-full max-w-5xl max-h-[85vh] bg-gray-900 bg-cover bg-center rounded-lg shadow-xl overflow-hidden" style={backdropStyle} onClick={(e) => e.stopPropagation()}>
                ...
                <div className="relative z-10 p-6 md:p-8 h-full flex flex-col md:flex-row gap-8 overflow-y-auto">
                    <div className="w-full md:w-1/3 flex-shrink-0">
                        <img 
                            src={formattedDetails?.poster_path || item.logo} 
                            alt={item.title} 
                    ...
                    <div className="text-white flex flex-col flex-grow">
                        ...
                        {isError && <p className="text-red-400">Não foi possível carregar os detalhes.</p>}
                        
                        {formattedDetails ? (
                            <>
                                <div className="flex items-center gap-4 mb-4">
                                    <StarRating rating={formattedDetails.rating} />
                                    {formattedDetails.release_date && (
                                        <span className="text-gray-300">
                                            Lançamento: {new Date(formattedDetails.release_date).toLocaleDateString('pt-BR', { timeZone: 'UTC' })}
                                        </span>
                                    )}
                                </div>
                                <p className="text-gray-300 mb-4 text-sm md:text-base max-h-32 overflow-y-auto">
                                    {formattedDetails.overview || 'Sinopse não disponível.'}
                                </p>
                                <p className="text-sm mb-2">
                                    <span className="font-bold">Elenco:</span> {formattedDetails.cast || 'Não informado'}
                                </p>
                                <p className="text-sm mb-4">
                                    <span className="font-bold">Gênero:</span> {formattedDetails.genres || 'Não informado'}
                                </p>
                            </>
                        ) : (
                            !isLoading && (
                                <div className="mb-4">
                                    <p className="text-gray-400 mb-4">Detalhes não disponíveis para este título.</p>
                                    {item.group && (
                                        <p className="text-sm">
                                            <span className="font-bold">Categoria:</span> {item.group}
                                        </p>
                                    )}
                                </div>
                            )
                        ...
                        <div className="mt-auto pt-6 flex flex-wrap items-center gap-4">
                            ...
                            {formattedDetails?.trailer_key && (
                                <button 
                                    onClick={() => window.open(`https://www.youtube.com/watch?v=${formattedDetails.trailer_key}`, '_blank')} 
                                    className="border border-white/50 text-white px-6 py-2 rounded font-bold hover:bg-white hover:text-black"
                                >
                                    Assistir Trailer
                                </button>
                            )}
                            ...
                            <button 
                                ...
                                className="border border-white/50 text-white px-6 py-2 rounded font-bold hover:bg-white hover:text-black"
                            >
                                {isItemFavorite ? '✓ Favorito' : '+ Favoritos'}
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <button 
                    onClick={onClose} 
                    className="absolute top-4 right-4 text-white text-3xl hover:text-cyan-400 transition-colors z-20"
                >
                    ×
                </button>
            </div>
